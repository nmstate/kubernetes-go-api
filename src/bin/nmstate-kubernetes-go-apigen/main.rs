...
fn try_main() -> Result<(), Error> {
    ...
    info!("output_file: {}", args.output_file);
    info!("header_file: {}", args.header_file);
    ...
    let mut visitor = RustVisitor {
        ...
        emitter: GolangEmitter {
            header: std::fs::read_to_string(&args.header_file)
                .context(format!("failed reading header file: {}", args.header_file))?,
            output: String::new(),
        },
    };
    ...
    let mut output_file = File::create(&args.output_file)
        .context(format!("failed creating output file {}", &args.output_file))?;
                ...
                _ => {
                    if let Some(discriminant) = node.discriminant.clone() {
                        ...
                        if let Expr::Lit(literal_discriminant) = expr {
                            if let Lit::Int(int_discriminant) = literal_discriminant.lit {
                                if let Ok(u8_discriminant) = int_discriminant.base10_parse::<u8>() {
                                    variant_ctx.u8_discriminant = Some(u8_discriminant);
                                }
                            }
                        }
                    }
        ...
        if let Some(current_struct) = self.current_struct.clone() {
            if let Some(field_ident) = node.clone().ident {
                let mut field_context = FieldContext {
                    ...
                    name: if field_ident == "iface_type" && current_struct.name == "BaseInterface" {
                        "type".to_string()
                    } else {
                        field_ident.to_string()
                    },
                    is_number_or_string: self
                        .has_serde_deserialize_with_number_or_string(&node.attrs),
                ...
                if let Some(renamed_field_name) = self.parse_serde_rename(&node.attrs) {
                    field_context.name = renamed_field_name;
                } else if let Some(struct_serde_rename_all) = current_struct.serde_rename_all {
...
"#,
        ...
        for file_to_visit in files_to_visit {
            let mut file =
                File::open(format!("{}/{}", input_dir, file_to_visit)).context(format!(
                    "failed opening file to visit {}/{}",
                    input_dir, file_to_visit
                ))?;
        ...
        if let (Some(linux_bridge_interface), Some(ovs_bridge_interface)) = (
            ...
            self.emitter
                .emit_bridge_interface(&linux_bridge_interface, &ovs_bridge_interface);
        }
            ...
            if let Some(node_ident) = node.ident.clone() {
                    ...
                    self.skip_fields.get(node_ident.to_string().as_str())
                {
                    return skip_field_structs.is_empty()
                        || skip_field_structs.contains(current_struct.name.as_str());
                } else {
                    return false;
                }
            }
    ...
    fn parse_field_type(&mut self, t: syn::Type, field_ctx: &mut FieldContext) {
        match t {
            ...
            _ => {
                self.emitter
                    .emit_type_name(String::from("todo"), &mut FieldContext::default());
            }
        }
    }
        ...
        if let Some(last_segment) = t.path.segments.last() {
                ...
                || last_segment_name == "Box"
            {
                ...
                if let syn::PathArguments::AngleBracketed(ab) = last_segment.arguments.clone() {
                    if let Some(syn::GenericArgument::Type(gt)) = ab.args.first() {
                ...
                if let syn::PathArguments::AngleBracketed(ab) = last_segment.arguments.clone() {
                    ...
                    if let Some(syn::GenericArgument::Type(key)) = ab.args.first() {
                        self.parse_field_type(key.clone(), &mut FieldContext::default());
                    }
                    self.emitter.emit_map_end();
                    if let Some(syn::GenericArgument::Type(value)) = ab.args.last() {
                        // The value of other_config and external_ids
                        // can be int or string
                        if matches!(field_context.name.as_str(), "other_config" | "external_ids") {
    ...
    fn parse_serde_alias(&mut self, attributes: &Vec<syn::Attribute>) -> Option<String> {
        self.parse_serde_attribute(attributes, "alias")
    }

    fn parse_serde_rename(&mut self, attributes: &Vec<syn::Attribute>) -> Option<String> {
    ...
    fn has_serde_skip_serialization(&mut self, attributes: &Vec<syn::Attribute>) -> bool {
        self.parse_serde_attribute(attributes, "skip_serializing")
            .is_some()
    }

    fn parse_serde_rename_all(&mut self, attributes: &Vec<syn::Attribute>) -> Option<String> {
        self.parse_serde_attribute(attributes, "rename_all")
    }

    fn parse_serde_deserialize_with(&mut self, attributes: &Vec<syn::Attribute>) -> Option<String> {
    ...
    fn parse_repr_attribute(&mut self, attributes: &Vec<syn::Attribute>) -> Option<String> {
                ...
                if let Some(first_segment) = list.path.segments.first() {
                    if first_segment.ident == key {
                        ...
                        while let Some(token) = tokens_iter.next() {
                            if let proc_macro2::TokenTree::Ident(iden_token) = token {
                                match attribute_name_op {
                                    Some(attribute_name) => {
                                        if iden_token == attribute_name {
                                            tokens_iter.next(); // =
                                            if let Some(rename_value) = tokens_iter.next() {
                                                return Some(
                                                    rename_value.to_string().replace('"', ""),
                                                );
                                            }
...
impl GolangEmitter {
    fn emit_enum_begin(&mut self, enum_ctx: &EnumContext) -> &mut GolangEmitter {
    ...
    fn emit_enum_variant(&mut self, variant_ctx: &VariantContext) -> &mut GolangEmitter {
        ...
        if !variant_ctx.enum_is_number_or_string && variant_ctx.u8_discriminant.is_some() {
            self.output.push('(');
            self.output
                .push_str(format!("{}", variant_ctx.u8_discriminant.unwrap()).as_str());
            self.output.push_str(")\n");
        } else {
            if variant_ctx.enum_is_number_or_string {
    ...
    fn emit_enum_variant_string(&mut self, variant_ctx: &VariantContext) -> &mut GolangEmitter {
        if let Some(variant_name) = variant_ctx.serde_rename.as_ref() {
            self.output.push_str(variant_name);
        } else if Some(String::from("kebab-case")) == variant_ctx.enum_serde_rename_all {
            self.output.push_str(&to_kebab_case(&variant_ctx.name));
        } else if Some(String::from("snake_case")) == variant_ctx.enum_serde_rename_all {
            self.output.push_str(&to_snake_case(&variant_ctx.name));
        } else if Some(String::from("lowercase")) == variant_ctx.enum_serde_rename_all {
    ...
    fn emit_enum_validation(&mut self, enum_ctx: &EnumContext) -> &mut GolangEmitter {
        ...
        for variant in &enum_ctx.variants {
            ...
            if variant.u8_discriminant.is_none() || variant.enum_is_number_or_string {
                self.output.push('"');
    ...
    fn emit_struct_begin(&mut self, struct_ctx: &StructContext) -> &mut GolangEmitter {
    ...
    fn emit_field_begin(&mut self, field_ctx: &FieldContext) -> &mut GolangEmitter {
...
pub fn rust_name_to_golang_name(s: &str) -> String {
    ...
    for word in to_snake_case(s).split('_') {
        let golang_word = match word {
            "ovn" | "ovs" | "uuid" | "db" | "udp" | "tcp" | "ip" | "dns" | "id" => {
                word.to_uppercase()
            }
            _ => capitalize(word),
        };
